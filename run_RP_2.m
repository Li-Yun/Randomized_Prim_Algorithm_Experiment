function run_RP_2(configFile, object_name, file_path, labl_files, image_files, nMaximumWindows, interval_number)
...
for window_array_index = 1: length(number_window_array)  
    ...
    % test each testing image and compute detected number over the testing image to   
    % each window number
    count_number = 0;
    ...
    for file_index = 1 : length(image_files)
        ...
        elseif (number_window_array(window_array_index) < length(proposals(:,1)))
            ...
            for repeat_index = 1 : 5
                ...
                % generate ID array
                %id_array = linspace(1, length(proposals(:,1)), length(proposals(:,1)));
                % randomly choose a sub-set from original ID array
                %ranId = randsample(id_array, number_window_array(window_array_index));
                ...
                if (ratio_result >= 0.5)
                    temp_array(repeat_index) = temp_array(repeat_index) + 1;
                end
    ...
    %disp(temp_array);
    %disp('===========');
    %detected_result(window_array_index).nWindow = number_window_array(window_array_index);
    %detected_result(window_array_index).detected_array = temp_array + count_number;
    detected_result(window_array_index,:) = temp_array + count_number;
    
end  % go through each window number.
